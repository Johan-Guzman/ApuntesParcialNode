...
describe("UserService", () => {
  ...
  it("debe crear un usuario", async () => {
    ...
    expect(UserModel.create).toHaveBeenCalledWith({ email: "new@test.com", password: "1234", name: "Mateo" });
  ...
  it("debe actualizar usuario", async () => {
    ...
    expect(UserModel.findOneAndUpdate).toHaveBeenCalledWith(
      { _id: "1" },
      { email: "update@test.com", name: "Mateo", password: "123" },
  ...
  it("debe devolver un token al hacer login", () => {
    const token = userService.login("login@test.com");
